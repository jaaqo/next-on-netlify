...
const getAllPages = () => {
  ...
  // ISR pages are pages in prerender manifest revalidate set
  const isrPages = Object.fromEntries(
    Object.entries(ssrAndHtmlPages)
      .map(([ssrRoute, ssrFilePath]) => {
        const isrPage = Object.entries(staticSsgPages).find(
          ([ssgRoute, { initialRevalidateSeconds }]) =>
            initialRevalidateSeconds && ssgRoute === ssrRoute
        );

        if (!isrPage) return;

        const [route, attrs] = isrPage;

        // Include file path for ISR pages from pages manifest
        return [route, { ...attrs, filePath: ssrFilePath }];
      })
      .filter(Boolean)
  );
  ...
  Object.entries(dynamicSsgPages).forEach(
    ([route, { dataRoute, fallback }]) => {
      ...
      // static SSG page block above or ISR pages
      if (fallback === false || route in isrPages) return;
  ...
  // Parse ISR pages
  Object.entries(isrPages).forEach(([route, { dataRoute, filePath }]) => {
    pages.push(
      new Page({
        route,
        type: "isr",
